...
# PD steering: derivative gain. 0.0 = disabled (pure P control).
STEER_KD: float = 0.0
_prev_steer_error: float = 0.0   # tracks previous error for derivative term

# Sensor averaging: when True, smooth_angle() blends 3 centre track rays.
SENSOR_AVG: bool = True

# Trail braking: when True, brake is scaled down proportional to steer demand.
TRAIL_BRAKE: bool = False


def smooth_angle(state: dict) -> float:
    """Return a smoothed heading angle using the three centre track rangefinder rays.

    Rays 7, 9, 11 correspond to -20°, 0°, +20°. The centre ray is weighted 2×.
    Falls back to raw state["angle"] if the track list is unavailable or too short.
    """
    raw = float(state.get("angle", 0.0))
    track = state.get("track")
    if not isinstance(track, (list, tuple)) or len(track) < 12:
        return raw
    # weighted average: centre ray weighted 2x, adjacent rays 1x each
    r7  = float(track[7]  or 0)   # -20 deg
    r9  = float(track[9]  or 0)   #   0 deg
    r11 = float(track[11] or 0)   # +20 deg
    total = r7 + 2 * r9 + r11
    if total <= 0:
        return raw
    # positive (r11 - r7) means more track ahead to the right → car angled left
    smoothed = raw + 0.03 * (r11 - r7) / max(r9, 1.0)
    return smoothed
...
def drive(state: dict) -> dict:
    global _prev_steer_error
    angle = smooth_angle(state) if SENSOR_AVG else float(state.get("angle", 0.0))
    ...
    if abs(track_pos) > 1.0:
        ...
        _prev_steer_error = 0.0
    else:
        error = angle - (track_pos - target_pos) * TRACK_POS_BIAS
        steer = (error + STEER_KD * (error - _prev_steer_error)) / STEER_LOCK
        _prev_steer_error = error
...
def drive_lookahead(state: dict) -> dict:
    ...
    if brake > 0.0:
        # Trail braking: reduce brake as steering increases (traction circle)
        if TRAIL_BRAKE and abs(steer) > 0.1:
            trail_factor = max(0.4, 1.0 - abs(steer) * 0.6)
            brake = brake * trail_factor
        accel = 0.0
    else:
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--laps", type=int, default=1, metavar="N",
                        help="Target lap count.  When > 1, MAX_STEPS scales to ~10000 per lap, "
                             ...
                             "Default: 1.")
    parser.add_argument("--steer-kd", type=float, default=0.0,
                        metavar="KD",
                        help="Derivative gain for PD steering control.  Adds a damping term "
                             "proportional to the change in steering error to reduce overshoot. "
                             "0.0 = disabled (pure P control, default).  Good starting value: 0.15.")
    parser.add_argument("--no-sensor-avg", action="store_true",
                        help="Disable 3-sensor angle averaging.  By default the driver blends "
                             "track rangefinder rays at -20°, 0°, +20° (indices 7/9/11) into a "
                             "smoothed heading estimate to reduce sensor noise.")
    parser.add_argument("--trail-brake", action="store_true",
                        help="Enable trail braking in lookahead mode.  Scales the brake pedal "
                             "down proportionally as steering demand increases, exploiting the "
                             "traction circle through corner entry.  Default: disabled.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    global TARGET_SPEED_KMH, SLOW_ZONES, SEGMENTS, LINE_PROFILE, LOOKAHEAD_M, LOOKAHEAD_DECEL_MS2, BRAKE_TEST_SPEED, FULL_PEDAL_BRAKE, MAX_STEPS, STEER_KD, SENSOR_AVG, TRAIL_BRAKE
    ...
    SENSOR_AVG = not args.no_sensor_avg
    TRAIL_BRAKE = args.trail_brake
                ...
                if BRAKE_TEST_STATE["done_ticks"] > 50:
                    ...
                    if logger is not None:
                        logger.log_frame(
                            ...
                            speed_target_kmh=target_speed_for(sensors),
